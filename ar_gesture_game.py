"""
AR Gesture Control for Games (Optimized - NO Center Action - Resizable Window)
- Creates a resizable OpenCV window using cv2.WINDOW_NORMAL.
- Center zone is a neutral/safe zone during gameplay.
- Reduced MediaPipe complexity for speed.
- Implements a Cooldown Timer for stable keyboard input.
"""
...
COMMAND_COOLDOWN = 0.3 # Time (in seconds) between key presses
LAST_COMMAND_TIME = time.time()
MODEL_COMPLEXITY = 0 # 0=LITE, 1=FULL (Use 0 for maximum speed)
...
cap.set(cv2.CAP_PROP_FPS, 60) # Attempt to set a high frame rate

# --- Create a resizable window ---
WINDOW_NAME = 'AR Gesture Game Control'
...
def draw_zones(frame, hand_center_x, hand_center_y, w, h, active_zone=None):
    ...
    # Draw horizontal lines for Up/Down/Center
    ...
    elif active_zone == 'center':
        cv2.rectangle(frame, (w_third, h_third), (w * 2 // 3, h * 2 // 3), HIGHLIGHT_COLOR, THICKNESS + 2)
        cv2.putText(frame, "CENTER", (w_third + w_third // 4, h // 2), FONT, 0.7, TEXT_COLOR, 2)
...
while cap.isOpened():
    ...
    active_zone_label = None # For visual highlighting
    ...
    if results.multi_hand_landmarks:
        ...
        if center_x < w_third:
            ...
            active_zone_label = 'left'
        elif center_x > w * 2 // 3:
            current_key_command = 'right'
            active_zone_label = 'right'
        elif w_third <= center_x <= w * 2 // 3:
            if center_y < h_third:
                current_key_command = 'up'
                active_zone_label = 'up'
            elif center_y > h * 2 // 3:
                current_key_command = 'down'
                active_zone_label = 'down'
            else:
                # Center Zone is neutral
                current_key_command = None
                active_zone_label = 'center'
                hand_in_center = True
    ...
    if not GAME_ACTIVE:
        ...
        draw_zones(frame, center_x if results.multi_hand_landmarks else -1, 
                          ...
                          w, h, active_zone=active_zone_label)
        
        # Send keyboard command with COOLDOWN
        # Send keyboard command with COOLDOWN
        if current_key_command and (time.time() - LAST_COMMAND_TIME > COMMAND_COOLDOWN):
            pyautogui.press(current_key_command)
            LAST_COMMAND_TIME = time.time()
        # Display current action
        cv2.putText(frame, f"ACTION: {current_key_command if current_key_command else 'NEUTRAL'}", 
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)


    # Final display and Exit
    cv2.imshow(WINDOW_NAME, frame) # Use the variable
